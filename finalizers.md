...
const text_finalizer =
	<Input, Output>(): LanguageModelFinalizer<Input, Output> => async (ctx) => {
		const content = ctx.history.at(-1)?.[1].at(-1)?.choices[0]?.message
			.content;
		if (!content) {
			throw new Error("No content in response");
		}
		// Return a simple object as the final output (adjust shape as needed)
		return { text: content } as unknown as z.output<Output>;
	};

// Custom finalizer with validation
const validated_finalizer = <Input, Output>(
	validate: (output: any) => boolean,
): LanguageModelFinalizer<Input, Output> =>
async (ctx) => {
	const content = ctx.history.at(-1)?.[1].at(-1)?.choices[0]?.message.content;
	if (!content) {
		throw new Error("No content in response");
	}
